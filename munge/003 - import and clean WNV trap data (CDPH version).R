...
cdph.wnv <- read.csv(paste(src.path,cdph.csv,sep="\\"),stringsAsFactors = FALSE)
...
write.csv(wnv.trap.date, paste(src.path,'wnv.trap.date.csv',sep='\\'),row.names = FALSE)
write.csv(wnv.trap.date.species, paste(src.path,'wnv.trap.date.species.csv',sep='\\'),row.names = FALSE)
...
wnv.zcta <- read.csv(paste(src.path,"GIS",'wnv_traps_zcta.csv',sep="\\"),stringsAsFactors = FALSE)
wnv.blkgrp <- read.csv(paste(src.path,"GIS",'wnv_traps_BG2018.csv',sep="\\"),stringsAsFactors = FALSE)
wnv.community.area <- read.csv(paste(src.path,"GIS",'wnv_traps_CommArea.csv',sep="\\"),stringsAsFactors = FALSE)
wnv.traps.zoning <- read.dbf(paste(src.path,"GIS",'wnv_traps_zoning.dbf',sep='\\'))
...
str(wnv.traps.zoning)
summary(wnv.traps.zoning$zone_class)
summary(wnv.traps.zoning$zone_type)
table(wnv.traps.zoning$zone_class,wnv.traps.zoning$zone_type)
...
wnv.trap.geo3 <- inner_join(wnv.trap.geo2,wnv.community.area[,c("trap_name","community")]
                            ,by=c("trap.name" = "trap_name"))
wnv.trap.geo4 <- inner_join(wnv.trap.geo3,wnv.traps.zoning[,c("trap_name","zone_class"
                                                              ,"zone_type")]
                            ,by=c("trap.name" = "trap_name"))

wnv.trap.geo4$zone_type <- as.factor(wnv.trap.geo4$zone_type)
wnv.trap.geo4$zone_class <- as.factor(wnv.trap.geo4$zone_class)
